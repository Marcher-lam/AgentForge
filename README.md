...
双层记忆架构，严格上下文预算控制（总计 ≤4400 chars / 2200 tokens）：

### 热层：ChatMemory（时序记忆）

Per-session 紧凑时序记忆，按 session_id 隔离，存储事件摘要（≤60字）而非原文：

```
[14:30] 用户: 前端框架选型
[14:31] 前端: React/Vue/Angular
[14:32] 前端→@程序员: 工程视角?
[14:33] 程序员(我): React工程化最强
[14:35] [共识] React大团队/Vue快速迭代
```

- 每条事件 ≤60 字摘要（vs 旧系统 500 字原文）
- 严格 ≤800 chars 预算注入 prompt
- 支持动作类型：问/答/@提及/PASS/共识
- 群聊A的记忆不会污染群聊B（session 隔离）
